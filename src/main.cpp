...
#if defined(NDEBUG)
# error "Tuppercoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Tuppercoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Tuppercoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Tuppercoin: find block wanted by given orphan block
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if(pindexPrev->nHeight < (1440 * 365 *1))
        nSubsidy = 15.4109589 * COIN;
    else if(pindexPrev->nHeight < (1440 * 365 *2))
        nSubsidy = 12.32876712 * COIN;
    else if(pindexPrev->nHeight < (1440 * 365 *3))
        nSubsidy = 9.24657534 * COIN;
    else if(pindexPrev->nHeight < (1440 * 365 *4))
        nSubsidy = 6.16438356 * COIN;
    else
...
// Tuppercoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Tuppercoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Tuppercoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Tuppercoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Tuppercoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Tuppercoin: track money supply and mint amount info
...
// Tuppercoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Tuppercoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Tuppercoin: compute stake entropy bit for stake modifier
    ...
    // Tuppercoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Tuppercoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Tuppercoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Tuppercoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Tuppercoin-loadblk");
